...
def plot(ax, title, dec_inter, disc, xtitle):
...
def get_pdbs_dict(input_dir, scoretype, rmsd_cutoff, rmsd, list_energies):
    dec = scorefileparse.read_dir(input_dir, scoretype, repl_orig=False, rmsd=rmsd, list_energies=list_energies)
...
def plot_pdbs_dict(axarr, x_ind, dec_inter, dec_unnorm, title,xtitle):

    for ind, pdb in enumerate(sorted(dec_inter.keys())):

        ax = axarr[ind,x_ind]

        plot(ax, title + " " + pdb, dec_inter[pdb], discparse.scores_dict_to_metrics(dec_unnorm[pdb]), xtitle)
...
def main(list_dirs, rmsd_cutoff, output_prefix, rmsd, list_energies):
    ...
    for d in list_dirs:
        scoretype = d[1]
        dec, dec_unnorm = get_pdbs_dict(d[0], scoretype, rmsd_cutoff, rmsd.get(scoretype), list_energies.get(scoretype)) #returns None if those values aren't present in dicts
        dec_list.append(dec)
	...
	#    xtitle="RMSD SUPLOOP"
            xtitle="RMSD"
	plot_pdbs_dict(axarr, ind, d, dec_u_list[ind], title_pre, xtitle)

    filename = output_prefix +  ".txt"

    suffix="energy_v_rmsd_{0}".format(rmsd_cutoff)

    y_vals = [ scorefileparse.get_energies(dec_scores) for dec_pdbs_dict in dec_list_inter for dec_scores in dec_pdbs_dict.values() ]
    x_vals = [ scorefileparse.get_rmsd(dec_scores) for dec_pdbs_dict in dec_list_inter for dec_scores in dec_pdbs_dict.values() ]
